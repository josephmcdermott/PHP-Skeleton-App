...
function show_register_form()
{
    ...
    $is_registered = $user_account->is_registered($_SESSION[$final_global_template_vars["session_key"]]["user_account_id"]);
    ...
    $app->render('register_form.php', array(
        "page_title" => false, "hide_side_nav" => true, "is_registered" => $is_registered, "groups" => $flat_group_hierarchy, "needs_group" => $needs_group, "submitted_data" => $app->request()->post(), "errors" => !empty($env["default_validation_errors"]) ? $env["default_validation_errors"] : false
    ));
}
